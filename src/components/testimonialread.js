...
export default function Testimonialread() {
    ...
    const onDelete = (id) => {
        axios.delete(`https://613a097f1fcce10017e78d33.mockapi.io/testData/${id}`)
     .then((res) => {
        setAPIData(res.data);
    })
}
            ...
            <Table singleLine>
                <Table.Header>
                    <Table.Row>
                        ...
                        <Table.HeaderCell>testimonialupdate</Table.HeaderCell>
       ...
       <Table.Row>
          <Table.Cell>{data.firstName}</Table.Cell>
           ...
           <Table.Cell>{data.checkbox ? 'Checked' : 'Unchecked'}</Table.Cell>
            <Link to='/testimonialupdate'>
            <Table.Cell>
             <Button onClick={() => setData(data)}>testimonialupdate</Button>
            </Table.Cell>
            </Link>
            <Table.Cell>
